...
class MoveOnSidesMixin:
    def check_sides(self,board,color,pos_x,pos_y,xy):
        if pos_x == xy[0] and pos_y != xy[1]:
            if color == Color.WHITE and 0 <= xy[1] <= 7:
                pos_y = xy[1]
                return True
            elif color == Color.BLACK and 0 <= xy[1] <= 7:
                pos_y = xy[1]
                return True
        elif pos_x != xy[0] and pos_y == xy[1]:
            if color == Color.BLACK and 0 <= xy[1] <= 7:
                pos_x = xy[0]
                return True
            elif color == Color.WHITE and 0 <= xy[1] <= 7:
                pos_y = xy[1]
                return True
        else:
...
class Piece(ABC):                                          #Класс шахматной фигуры(родительский)
    IMG = None
...
class Rook(Piece, MoveOnSidesMixin):                                      #Класс ладьи, наследуемый от класса фигуры
    IMG = ('♖', '♜')
    ...
    def move(self, board):                              #Функция для движения ладьи
        ...
        while a==False:
            ...
            xy = []
            xy = [int(i) for i in input().split()]
            a = self.check_sides(board,self.color,self.pos_x,self.pos_y,xy)
        self.pos_y = xy[1]
        self.pos_x = xy[0]
        return self.pos_x, self.pos_y
